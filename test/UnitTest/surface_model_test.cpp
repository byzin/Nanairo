...
TEST(SurfaceModelTest, SmoothDiffuseSurfaceTest)
{
  ...
  const UniquePointer<Texture> texture{new ValueTexture{*system, settings, prefix}};
  ...
  const UniquePointer<SurfaceModel> surface{
      new SmoothDiffuseSurface{settings, prefix, texture_list}};
  // Intersection point
...
void testRoughConductorSurface(const nanairo::Float roughness)
{
  ...
  settings.setIntValue(key, 0);
  const UniquePointer<SurfaceModel> surface{
      new RoughConductorSurface{settings, prefix, texture_list}};
  // Intersection point
  const Point3 point{0.0, 0.0, 0.0};
  const Vector3 normal{0.0, 1.0, 0.0};
  const IntersectionInfo intersection{point, normal, nullptr, false};
