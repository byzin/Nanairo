/*!
  \file spectral_distribution_spectra.hpp
  \author Sho Ikeda
...
#ifndef NANAIRO_SPECTRAL_DISTRIBUTION_SPECTRA_HPP
#define NANAIRO_SPECTRAL_DISTRIBUTION_SPECTRA_HPP
...
class SpectralDistributionSpectra : public SpectralDistribution
{
 public:
  using DataType = std::conditional_t<kCompensated,
                                      zisc::CompensatedSummation<Float>,
                                      Float>;
  using DistributionType = std::array<DataType, CoreConfig::spectraSize()>;


  //! Initialize a distribution with 0
  SpectralDistributionSpectra() noexcept;

  //! Create a distribution
  SpectralDistributionSpectra(const DistributionType& property) noexcept;
 ...
 private:
  //! 
  void setAsSpectraColor(const System& system,
                         const SpectralDistribution& other,
                         zisc::pmr::memory_resource* work_resource) noexcept;

  //!
  void setAsSpectraData(const SpectralDistribution& other) noexcept;
...
// Type aliases
using SpectraDistribution = SpectralDistributionSpectra<false>;
using CompensatedSpectraDistribution = SpectralDistributionSpectra<true>;

//! \} Core

} // namespace nanairo

#endif // NANAIRO_SPECTRAL_DISTRIBUTION_SPECTRA_HPP
